...
import {
  ...
  favTrack, pauseTrack, playTrack, seekTrack, nextTrack, prevTrack
} from '../State';
...
const TrackProgress = ({ playing, track, onSeek }: any) => {
  const progress = playing.progress;
  const left = track.time - progress;
  const percent = (progress / track.time) * 100;

  const s = (p: RangeValue) => {
    const newTime = (p as any / 100) * track.time;
    onSeek(newTime);
  }
  return (
...
const TrackControls = ({ playing, isFav, onPause, onPlay, onPrev, onNext, onFav }: any) => {
  return (
...
const TrackPlayer = ({ track, closed }: any) => {
